...
interface CategoryDefDao {
    ...
    /** 根据名称和父级查找分类 */
    @Query("SELECT * FROM category_def WHERE name = :name AND (parent_id = :parentId OR (parent_id IS NULL AND :parentId IS NULL))")
    suspend fun getCategoryByName(name: String, parentId: Long?): CategoryDefEntity?

    /** 根据名称、父级和 flowSign 查找分类（用于去重检查） */
    @Query("SELECT * FROM category_def WHERE name = :name AND (parent_id = :parentId OR (parent_id IS NULL AND :parentId IS NULL)) AND flow_sign = :flowSign")
