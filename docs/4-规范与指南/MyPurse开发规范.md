...
6. docs/4-规范与指南/MyPurse开发规范.md（重点关注 2.3、11、14.6 节）
...
| V1.16 | 2026-06-21 | codebuddy | **执行层保障机制**：①2025.2.3 新增「Agent 收尾必检清单」— 5 条精简必检项（日志/打勾/问题记录/需求变更/临时文件），解决 9 条收尾规则实际执行遗漏问题；②13.2「Agent 开发结束后必须」新增第 6 条（子阶段完成必须打勾）和第 9 条（收尾后对照必检清单自检）；③`开发阶段.md` 阶段 10 四个子阶段表格统一补加"状态"列；④10d-1、10d-2 打勾（颜色体系已在会话 #33 完成但未记录）；⑤原 13.2.4 版本迭代规范 → 13.2.5 |