...
| 开发日志 | `开发日志.md` | `docs/3-开发过程/V1.X/`（按版本分目录） |
| 问题/Bug 记录 | `问题记录.md` | `docs/3-开发过程/V1.X/`（按版本分目录） |
...
**给 Agent 的初始提示应包括**：详见下方 13.2.1 节「Agent 初始提示模板」。

**Agent 开发结束后必须**（遵循通用开发规范第 0 章）：
1. 更新 `docs/3-开发过程/V1.X/开发日志.md`（记录本次改动）
2. 同步更新 `docs/3-开发过程/开发日志-精简版.md`（跨版本统一）
3. 如有 Bug 修复 → 更新 `docs/3-开发过程/V1.X/问题记录.md`
4. 同步更新 `docs/3-开发过程/问题记录-精简版.md`（跨版本统一）
5. 如阶段完成 → 更新 `docs/3-开发过程/开发阶段.md`
6. **任何子阶段项完成后 → 在 `开发阶段.md` 对应表格的状态列打 ✅**（强制）
7. 如有需求变更 → 更新 `docs/2-需求规格/requirements.md` 正文 + 第 10 章变更记录（含原文摘要和决策理由）
8. 清理临时文件（编译输出、调试日志等）
9. **收尾完成后对照 13.2.3 节「Agent 收尾必检清单」逐条自检**（强制）
...
| 《通用开发规范》0.3 编译验证 | 每次改动后编译 | 基本保障（14.6 正文已丢失，功能由本章覆盖） |
...
6. docs/2-需求规格/requirements.md（V1.0 功能规格）
7. docs/2-需求规格/requirements-v1x.md（V1.x 迭代需求）
8. docs/4-规范与指南/MyPurse开发规范.md（重点关注 2.3、11 节）
9. docs/3-开发过程/开发阶段.md（当前进度）
10. docs/4-规范与指南/通用开发规范.md（重点关注 0.3 节）
```
...
#### 13.2.3 Agent 收尾必检清单（强制）

> **目的**：9 条收尾规则在实际执行中容易被遗漏。本节将其精简为 5 条必检项，Agent 每次会话结束前**必须逐条自检并完成**。

```
□ 1. 开发日志已追加本次会话记录（含修改文件清单 + 设计决策）
□ 2. 开发阶段.md 中涉及的子阶段项状态列已打勾 ✅/🔲
□ 3. 如有 Bug 修复 → 问题记录已更新（含模板完整填写）
□ 4. 如有需求/设计变更 → requirements 正文 + 第10章变更记录已更新
□ 5. 临时文件已清理（编译输出、调试日志等）
```
...
| # | 检查项 | 判断标准 | 常见遗漏场景 |
|---|--------|---------|------------|
| 1 | 开发日志 | 日志末尾有本次会话记录，包含日期、会话编号、修改文件、关键决策 | 只改了代码忘了记日志 |
| 2 | 进度打勾 | 打开 `开发阶段.md`，找到本次涉及的任务项，状态列为 ✅ 或 🔲 | **高频遗漏**：代码写完了但表里没打勾 |
| 3 | 问题记录 | 修复了 Bug 但没在对应版本目录的 `问题记录.md` 中追加记录 | 只修不记，后人不知踩过什么坑 |
| 4 | 需求变更 | 改变了交互行为/设计决策但没更新需求文档 | 即兴改动没回写 |
| 5 | 临时文件 | 工作区中无 `*.tmp`、`compile_*.txt` 等调试产物 | 编译排查后忘删 |

> **注意**：精确规则仍以 13.2 节 9 条"Agent 开发结束后必须"为准，本清单是其**精简执行版**。冲突时以 9 条为准。
...
| 完整版本 | 精简版本 | 用途 |
|----------|----------|------|
| `V1.X/开发日志.md` | `开发日志-精简版.md`（跨版本统一） | 快速了解项目开发历程 |
| `V1.X/问题记录.md` | `问题记录-精简版.md`（跨版本统一） | 快速定位问题和解决方案 |
...
| **问题记录** | **新版本新目录，独立编号** | V1.0 问题在 `V1.0/问题记录.md`（V1.0-#1 ~ #39），V1.1 问题在 `V1.1/问题记录.md`（V1.1-#1 ~ #9）。各版本独立文件，各自编号从 #1 开始 |
| **开发日志** | **新版本新目录，独立记录** | V1.0 日志在 `V1.0/开发日志.md`（会话 #1~#42），V1.1 日志在 `V1.1/开发日志.md`（会话 #1~#4），V1.2 日志在 `V1.2/开发日志.md`（会话 #1~#2）。各版本独立编号，各自从 1 开始 |
| **开发阶段** (`开发阶段.md`) | **同文件追加新阶段** | 后续优化任务追加为"阶段 11"，接在阶段 10 之后 |
...
| 更新日志 | `docs/3-开发过程/V1.X/开发日志.md` | 记录每个版本的改动（按版本分目录） |
...
| V1.16 | 2026-06-21 | codebuddy | **执行层保障机制**：①13.2.3 新增「Agent 收尾必检清单」— 5 条精简必检项（日志/打勾/问题记录/需求变更/临时文件），解决 9 条收尾规则实际执行遗漏问题；②13.2「Agent 开发结束后必须」新增第 6 条（子阶段完成必须打勾）和第 9 条（收尾后对照必检清单自检）；③`开发阶段.md` 阶段 10 四个子阶段表格统一补加"状态"列；④10d-1、10d-2 打勾（颜色体系已在会话 #33 完成但未记录）；⑤原 13.2.4 版本迭代规范 → 13.2.5 |